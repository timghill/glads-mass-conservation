...
pt.out_t = pt.start:30*86400:pt.end;

addpath(genpath('../data'))
pin.source_term_s = make_anon_fn('@(xy, time) double(0.05/86400/365 + 0*xy(:, 1));');
m_moulin = moulin_inputs(dmesh.tri, 50, 0);
pin.source_term_c = make_anon_fn('@(time) m_moulin', m_moulin);
